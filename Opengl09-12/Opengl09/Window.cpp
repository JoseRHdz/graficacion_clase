...
void Window::startup(string title, string pathShaderV, string pathShaderF,string pathObj, string pathTex)
{
    this->title = title;
    initGLFW();
    initGLEW();
    program.startup(pathShaderV,pathShaderF);        
    geom.startup(pathObj,pathTex);
}
...
{    
    ...
    transform = rotate(transform, radians(1.f), vec3(0.0f, 1.0f, 0.0f));
